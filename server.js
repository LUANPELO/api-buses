...
// 🧪 ENDPOINT PARA CREAR DATOS DE PRUEBA DE PAGOS
app.post("/test/create-sample-payments", async (req, res) => {
  try {
    const samplePayments = [
      {
        id: "PAY_1699123456789_abc123def",
        mercadopago_id: "MP_1699123456789",
        reservation_id: "TICKET_1699123456789_xyz789abc",
        amount: 45000,
        method: "card",
        status: "approved",
        created_at: "2024-01-15T10:30:00.000Z",
        processed_at: "2024-01-15T10:30:15.000Z",
        details: {
      ...
      {
        id: "PAY_1699123456790_def456ghi",
        mercadopago_id: "PSE_1699123456790",
        reservation_id: "TICKET_1699123456790_uvw456def",
        amount: 92000,
        method: "pse",
        status: "approved",
        created_at: "2024-01-15T11:45:00.000Z",
        processed_at: "2024-01-15T11:45:30.000Z",
        details: {
          bank_id: "1007",
      ...
      {
        id: "PAY_1699123456791_ghi789jkl",
        mercadopago_id: "MP_1699123456791",
        reservation_id: "TICKET_1699123456791_rst123ghi",
        amount: 47000,
        method: "card",
        status: "rejected",
    ...
    const existingPayments = await readJSONFile(paymentsFile);
    
    // Agregar pagos de ejemplo (evitar duplicados)
    const newPayments = [...existingPayments];
    
    samplePayments.forEach(samplePayment => {
      const exists = existingPayments.find(p => p.id === samplePayment.id);
      if (!exists) {
        newPayments.push(samplePayment);
      }
    });

    // Guardar
    await writeJSONFile(paymentsFile, newPayments);

    res.json({
      success: true,
      message: "Datos de prueba creados",
      created_samples: samplePayments.length,
      total_payments: newPayments.length,
      note: "Ahora puedes visitar /payments para ver los datos"
    });
...
// 🧹 ENDPOINT PARA LIMPIAR DATOS DE PAGOS (SOLO PARA DESARROLLO)
app.delete("/test/clear-payments", async (req, res) => {
  try {
    await writeJSONFile(paymentsFile, []);
    
    res.json({
      success: true,
      message: "Todos los pagos han sido eliminados",
      note: "El archivo payments.json ha sido vaciado"
    });
    ...
    res.status(500).json({
      success: false,
      error: "Error limpiando pagos",
      details: error.message
    });
  }
});

// 📊 MEJORAR EL ENDPOINT /payments PARA MOSTRAR MÁS INFO CUANDO ESTÁ VACÍO
app.get("/payments", async (req, res) => {
  try {
    console.log("🔍 Solicitando todos los pagos...");
    console.log("📁 Ruta del archivo:", paymentsFile);
    
    // Verificar que el archivo existe
    if (!fs.existsSync(paymentsFile)) {
      console.log("⚠️ Archivo payments.json no existe, creándolo...");
      await writeJSONFile(paymentsFile, []);
    }
    ...
    res.json({
      ...
      message: payments.length === 0 ? 
        "No hay pagos registrados aún. Los pagos aparecerán aquí cuando uses /process-payment o crees datos de prueba con /test/create-sample-payments" : 
        `${payments.length} pagos encontrados`,
      file_path: paymentsFile,
      timestamp: new Date().toISOString(),
      // Agregar enlaces útiles si está vacío
      ...(payments.length === 0 && {
        helpful_links: {
          create_sample_data: "/test/create-sample-payments (POST)",
          process_payment: "/process-payment (POST)",
          debug_files: "/debug/files (GET)"
        }
      })
    });
  } catch (error) {
